...
module.exports = {
    ...
    run: async (Client, Interaction) => {
        ...
        const createComponents = (page) => {
            ...
            // Container principal avec accent color
            const mainContainer = new ContainerBuilder()
                .setAccent('#0099FF')
                .setId('main_help_container');

            // Section d'en-tête avec thumbnail
            const headerSection = new SectionBuilder()
                .setId('header_section')
                ...
                .setThumbnailAccessory(
                    thumbnail => thumbnail
                        .setURL('https://cdn.discordapp.com/emojis/1234567890123456789.png')
                        .setDescription(lang === 'ar' ? 'أيقونة البوت' : 'Bot Icon')
                        .setClickable(false)
                );
            ...
            if (protectionCommands.length > 0) {
                commandsContent += `## ${lang === 'ar' ? '🛡️ أوامر الحماية' : '🛡️ Protection Commands'}\n`;
                protectionCommands.forEach(cmd => {
                    commandsContent += `**/${cmd.name}** - ${cmd.description}\n`;
                });
                commandsContent += '\n';
            }
            
            if (moderationCommands.length > 0) {
                commandsContent += `## ${lang === 'ar' ? '🔨 أوامر الإشراف' : '🔨 Moderation Commands'}\n`;
            ...
            if (utilityCommands.length > 0) {
                commandsContent += `## ${lang === 'ar' ? '⚙️ أوامر المساعدة' : '⚙️ Utility Commands'}\n`;
            ...
            if (commandsContent === '') {
                commandsContent = `## ${lang === 'ar' ? '📋 جميع الأوامر' : '📋 All Commands'}\n`;
                for (let i = start; i < end; i++) {
                    commandsContent += `**/${commands[i].name}** - ${commands[i].description}\n`;
                }
            }

            const commandsText = new TextDisplayBuilder()
                .setContent(commandsContent);

            commandsSection.addTextDisplayComponents(commandsText);
            ...
            const infoSection = new SectionBuilder()
                ...
                .addTextDisplayComponents(
                    textDisplay => textDisplay
                        .setContent(`> ${lang === 'ar' ? '💡 **نصيحة:** استخدم الأزرار أدناه للتنقل بين الصفحات' : '💡 **Tip:** Use the buttons below to navigate between pages'}`)
                );

            // Ajouter tous les composants au container principal
